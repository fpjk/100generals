    ...
    : TriggerSkill("game_rule")
{
    ...
    events << GameStart << TurnStart
        ...
        << ConfirmDamage << DamageInflicted << PreDamageDone << DamageDone << DamageComplete
...
bool GameRule::trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
{
    ...
    if (player == NULL) {
        if (triggerEvent == GameStart) {
            ...
            foreach (ServerPlayer *player, room->getPlayers()) {
                ...
                if (mode != "06_3v3" && mode != "06_XMode" && mode != "02_1v1" && !player->isLord()){
                    room->setPlayerMark(player, "@yuan", 1);
                }
            }
            room->setTag("FirstRound", true);
            bool kof_mode = room->getMode() == "02_1v1";
            QList<int> n_list;
            foreach (ServerPlayer *p, room->getPlayers()) {
    ...
    case EventPhaseChanging: {
        ...
        if (change.to == Player::NotActive) {
            ...
            if (!player->getPile("ji").isEmpty()) {
                DummyCard *dummy = new DummyCard(player->getPile("ji"));
                CardMoveReason reason(CardMoveReason::S_REASON_EXCHANGE_FROM_PILE, player->objectName(), "ji", QString());
    ...
    case DamageInflicted: {
        DamageStruct damage = data.value<DamageStruct>();
        QString mode = room->getMode();
        if (mode != "06_3v3" && mode != "06_XMode" && mode != "02_1v1" && damage.to->isLord()){
            foreach (ServerPlayer *player, room->getOtherPlayers(damage.to)) {
                if (damage.damage >= damage.to->getHp() && player->getMark("@yuan")>0 && room->askForSkillInvoke(player, "jiuzhu", "yes")){
                    room->removePlayerMark(player, "@yuan");
                    room->loseHp(player);
                    --damage.damage;
                    data = QVariant::fromValue(damage);
                    if (damage.damage < 1)
    ...
    case SlashEffected: {
        ...
        room->getThread()->trigger(SlashProceed, room, effect.to, data);            
        break;
    }
    case SlashProceed: {
        SlashEffectStruct effect = data.value<SlashEffectStruct>();
...
void GameRule::changeGeneral1v1(ServerPlayer *player) const
{
    ...
    QList<ServerPlayer *> notified = room->getPlayers();
...
void GameRule::rewardAndPunish(ServerPlayer *killer, ServerPlayer *victim) const
{
    ...
    if (killer->getRoom()->getMode() == "06_3v3") {
        ...
        if (victim->getRole() == "rebel" && killer != victim && !killer->hasFlag("huilei"))
            killer->drawCards(2, "kill");
