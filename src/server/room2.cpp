...
bool Room::useCard(const CardUseStruct &use, bool add_history)
{
    ...
    bool slash_not_record = Sanguosha->correctCardTarget(TargetModSkill::Residue, card_use.from, card) > 500;
...
void Room::recover(ServerPlayer *player, const RecoverStruct &recover, bool set_emotion)
{
    ...
    if (thread->trigger(PreHpRecover, this, player, data) || player->getMark("hunshang") > 0)
        return;
