...
void Dashboard::selectAll()
{
    foreach (const QString &pile, Self->getPileNames()) {
        if (pile.startsWith("&") || pile == "wooden_ox" || (Self->getPhase() == Player::NotActive && pile == "ji"))
...
void Dashboard::retractAllSkillPileCards()
{
    foreach (const QString &pileName, _m_pile_expanded) {
        if (!(pileName.startsWith("&") || pileName == "wooden_ox" || (Self->getPhase() == Player::NotActive && pileName == "ji")))
