...
void Drowning::onEffect(const CardEffectStruct &effect) const
{
    ...
    if (!to->getEquips().isEmpty())
        choices << "throw";
    int x = to->getHp();
    choices << "damage:::" + QString::number(x);
    if (room->askForChoice(to, objectName(), choices.join("+"), QVariant::fromValue(effect)) == "throw")
        to->throwAllEquips();
    else {
        if (to->getHandcardNum() < x || !room->askForDiscard(to, objectName(), x, x, true))
            room->damage(DamageStruct(objectName(), effect.from->isAlive() ? effect.from : NULL, to, 1, DamageStruct::Thunder));
    }
}
    ...
    : Package("maneuvering", Package::CardPack)
{
    ...
    cards << new Fan(Card::Diamond, 1)
        << new Triblade(Card::Diamond, 2)
        << new Analeptic(Card::Diamond, 3)
    ...
    OffensiveHorse *hualiu = new OffensiveHorse(Card::Diamond, 13);
    ...
    skills << new GudingBladeSkill << new FanSkill
        << new VineSkill << new SilverLionSkill << new TribladeSkill;
