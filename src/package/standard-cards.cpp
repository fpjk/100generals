...
bool Slash::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const
{
    ...
    if (targets.length() >= slash_targets) {
        if (Self->hasSkill("duanbing")) {
            int duanbing_extra = qMax(Self->getLostHp()-1, 0);
            //int specific_occupied = 0;
            if (targets.length() > slash_targets + duanbing_extra) return false;
            //QList<const Player *> duanbing_targets;
            int duanbing_num = 0;
            //bool no_other_assignee = true;
            foreach (const Player *p, targets) {
                if (Self->distanceTo(p, rangefix) == 1 && !Slash::IsSpecificAssignee(p, Self, this))
                    duanbing_num++;
            ...
            return duanbing_num >= targets.length() + 1 - slash_targets || Self->distanceTo(to_select, rangefix) == 1;
        } else
            return false;
    }
...
{
    int total_num = 1 + Sanguosha->correctCardTarget(TargetModSkill::ExtraTarget, Self, this);
    bool include_judging = !(ServerInfo.GameMode == "02_1v1");
...
void Snatch::onEffect(const CardEffectStruct &effect) const
{
    ...
    Room *room = effect.to->getRoom();
    bool using_2013 = (room->getMode() == "02_1v1");
...
void WoodenOxCard::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &) const
{
    ...
    if (targets.isEmpty())
        return;
    ServerPlayer *target = room->askForPlayerChosen(source, targets, "wooden_ox", "@wooden_ox-move");
    if (!target) {
    ...
    : Package("standard_cards", Package::CardPack)
{
    ...
    skills << new DoubleSwordSkill << new QinggangSwordSkill
        ...
        << new HalberdSkill << new RenwangShieldSkill << new IceSwordSkill
...
CROSSBOWPackage::CROSSBOWPackage()
    : Package("CROSSBOW", Package::CardPack)
{
    QList<Card *> cards;
