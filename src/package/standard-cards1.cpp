...
#define THIS_SLASH (slash == NULL ? newslash : slash)
    ...
    if (player->getPhase() == Player::Play && Sanguosha->getCurrentCardUseReason() == CardUseStruct::CARD_USE_REASON_PLAY) {
        ...
        if (player->canSlashWithoutCrossbow(THIS_SLASH))
            return true;
        int used = player->getSlashCount();
        int valid = 1 + Sanguosha->correctCardTarget(TargetModSkill::Residue, player, newslash);
        if ((!ids.contains(player->getWeapon()->getEffectiveId()) && player->hasWeapon("crossbow")) && used < valid + 3)
            return true;
...
{
    int total_num = 1 + Sanguosha->correctCardTarget(TargetModSkill::ExtraTarget, Self, this);
    if (targets.length() >= total_num || to_select->getCardCount(true, true) == 0 || to_select == Self)
        return false;
...
{
    int total_num = 1 + Sanguosha->correctCardTarget(TargetModSkill::ExtraTarget, Self, this);
    return targets.length() < total_num && to_select->getCardCount(true, true) > 0 && to_select != Self;
}
...
{
    QList<Card *> cards;
    cards << new WoodenOx(Card::Spade, 2);
